...
class ColorCubit extends Cubit<ColorState>{
  ...
  : super (ColorCurrentState(color: Colors.transparent, 
  isLoading: true)){
    getColor();
  }
  ...
  void getColor() async{
    ...
    result.fold((failure) => emit(ColorCurrentState(color: state.color, 
    isLoading: false, 
  ...
  void setColor(Color color) async{
    ...
    result.fold((failure) => emit(ColorCurrentState(color: state.color, isLoading: false, error: failure.getlocalizedString())),
    (entity)=>emit(ColorCurrentState(
      isLoading:false, 
      color: Color(
        int.tryParse(entity.color)?? 0xFFFFFFF,
